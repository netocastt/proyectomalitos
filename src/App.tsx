...
function safeGetItem(key: string): string | null {
  try {
    ...
    console.warn("No se pudo acceder a localStorage (posiblemente bloqueado por iframe o dominio):", e);
    return null;
  }
}
...
function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn("No se pudo guardar en localStorage (posiblemente bloqueado por iframe o dominio):", e);
  }
}
...
export default function App() {
  const [authMode, setAuthMode] = useState<AuthMode>(() => {
    const saved = safeGetItem('studyzen_auth');
  ...
  const [user, setUser] = useState<UserProfile>(() => {
    const saved = safeGetItem('studyzen_user');
  ...
  const [sessionConfig, setSessionConfig] = useState<SessionConfig>(() => {
    const saved = safeGetItem('studyzen_config');
    return saved ? JSON.parse(saved) : {
      focusTime: 25,
      shortBreak: 5,
      longBreak: 15
  ...
  const [tasks, setTasks] = useState<Task[]>(() => {
    const saved = safeGetItem('studyzen_tasks');
    return saved ? JSON.parse(saved) : [
  ...
  // Persistencia
  useEffect(() => {
    safeSetItem('studyzen_tasks', JSON.stringify(tasks));
  }, [tasks]);

  useEffect(() => {
    safeSetItem('studyzen_auth', authMode);
    safeSetItem('studyzen_user', JSON.stringify(user));
    safeSetItem('studyzen_config', JSON.stringify(sessionConfig));
